...
from models.gmflow_model.utils import merge_splits, split_feature, CustomLayerNorm
...
def single_head_split_window_attention(
    ...
    if flash_attention is True:
        ...
        k = k.view(b_new, -1, c)
        v = v.view(b_new, -1, c)
        qkv = torch.stack([q, k, v], dim=2) # [B*K*K, H/K*W/K, 3, C]
        qkv = torch.stack([q, k, v], dim=2).unsqueeze(3) # [B*K*K, H/K*W/K, 3, 1, C]
        out = flash_attn_qkvpacked_func(qkv=qkv, dropout_p=0.0) # [B*K*K, H/K*W/K, 1, C]
        out = out.squeeze(2) # [B*K*K, H/K*W/K, C]
    else:
        scores = (
        ...
        **kwargs,
        ...
        for layer in self.layers:
            if self.gradient_checkpointing:
                concat0 = checkpoint.checkpoint(self.custom(layer), 
                    concat0,
                    concat1,
                    h,
                    w,
...
class FeatureFlowAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # q, k: feature [B, C, H, W], v: flow [B, C, H, W]
        ...
        q = self.q_proj(query)  # [B, H*W, C]
        k = self.k_proj(query)  # [B, H*W, C]

        value = flow.view(b, flow.size(1), h * w).permute(0, 2, 1)  # [B, H*W, C]

        scores = torch.matmul(q, k.permute(0, 2, 1)) / (c**0.5)  # [B, H*W, H*W]
        prob = torch.softmax(scores, dim=-1)

        out = torch.matmul(prob, value)  # [B, H*W, C]
        out = out.view(b, h, w, value.size(-1)).permute(0, 3, 1, 2)  # [B, C, H, W]
...
class AffinityTransformerLayer(nn.Module):
    def __init__(
        self,
        q_dim=256,
        kv_dim=1024,
        ...
        **kwargs,
        ...
        super(AffinityTransformerLayer, self).__init__()
        ...
        self.merge = nn.Linear(q_dim, q_dim, bias=False)
        ...
        if not self.no_ffn:
            ...
            self.mlp = nn.Sequential(
                nn.Linear(in_channels, in_channels * ffn_dim_expansion, bias=False),
                nn.GELU(),
                nn.Linear(in_channels * ffn_dim_expansion, q_dim, bias=False),
            )
...
class AffinityTransformerBlock(nn.Module):
    """self attention + cross attention + FFN"""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(AffinityTransformerBlock, self).__init__()
        d_model = q_dim
        ...
        self.affinity_cross_attn_ffn = AffinityTransformerLayer(
            q_dim=q_dim,
            kv_dim=kv_dim,
            nhead=nhead,
            attention_type=attention_type,
    ...
    def forward(
        ...
        **kwargs,
        ...
        target = target.permute(0, 2, 1)  # (B, C, L)

        c = source.shape[2]

        temperature = c**0.5 if self.temperature == 0 else self.temperature

        # (B, L, L)
        correlation = torch.matmul(source, target) / temperature
        probabilites = F.softmax(correlation, dim=-1)

        source = self.affinity_cross_attn_ffn(
            source,
            probabilites,
            height=height,
            width=width,
...
class AffinityFeatureTransformer(nn.Module):
    def __init__(
        self,
        num_layers=2,
        q_dim=128,
        ...
        **kwargs,
        ...
        super(AffinityFeatureTransformer, self).__init__()
